...
var teamApi = (function() {
    ...
    var fetch = function(params) {
       ...
       if(Ti.App.Properties.hasProperty('auth')){
            ...
            Ti.API.debug("Teamwork Request: "+url)
            xhr.onload = function(e) {
                e.status = this.status;
                if (this.status == 200 || this.status == 201) {
                    Ti.API.debug("Teamwork Response: "+this.responseText);
                    onload && onload(JSON.parse(this.responseText));
                } else if(this.status === 204){
                    Ti.API.debug("Teamwork Response: NO CONTENT");
                    onload && onload("No Available Content");
                    
                } else{
                    Ti.API.debug("Teamwork Error: "+this.status+"("+e+")");
                    this.onerror(e);
                }
            };
            
            xhr.onerror = function(e) {
                Ti.API.debug("Teamwork Request Error: "+this.status);
                e.status = this.status;
                onerror && onerror(e);
            };
            ...
            Ti.API.error("TeamworkPM API Key Required.");
        }
    };
    ...
    publicMethods.setAuth = function(params){
        if(params && params!="clear"){
            if(params.apiKey){
                 ...
                 Ti.API.debug("Teamwork API Key Set");
            }
        } 
    };
    
    publicMethods.checkAuth = function(params){
    ...
    publicMethods.setResponseType = function(params){
        if(params && params=="json"){
            ...
            Ti.API.error("MODULE ti.teamwork responseType accepts 'json' or 'xml' only.");
        }
    };
        ...
        if(API[_method].url.indexOf("{id}")!=-1 && API[_method].url.indexOf("{alt_id}")!=-1){
             ...
             publicMethods[_method] = function(params){
                if (params && params.id && params.alt_id) {
                    ...
                    Ti.API.error("MODULE ti.teamwork: Missing required parameter 'alt_id' for "+_method);
                } else if(params && !params.alt_id){
                    Ti.API.error("MODULE ti.teamwork: Missing required parameter 'id' for "+_method);
                } else {
                    Ti.API.error("MODULE ti.teamwork: Missing parameters for "+_method);
                }
            };
